...
- **presentation/** — mount; tabs: **Mine**, **Empire**, **Research**, **Upgrades**, **Stats**; views (statsView, upgradeListView, planetListView, questView, comboView, progressionView, prestigeView, crewView, statisticsView, researchView, housingView). Toasts, tooltip, StarfieldCanvas, MineZoneCanvas. **presentationPortImpl** (createPresentationPort wires real UI to the port). Reusable UI in **presentation/components/**.
