...
const stringsEn = {
  ...
  planets: 'Planets',
  planetsHint: 'Each planet has upgrade slots (expandable). More planets = +5% production each. Send astronauts on an expedition to discover a new planet (some may die); if all survive or at least one returns, you discover it. Add slots or build housing on a planet (+2 crew capacity per module, uses 1 slot).',
  housing: 'Housing',
...
const stringsFr: Record<keyof typeof stringsEn, string> = {
  ...
  planets: 'Planètes',
  planetsHint: 'Chaque planète a des emplacements d’amélioration (extensibles). Plus de planètes = +5 % de production chacune. Envoyez des astronautes en expédition pour en découvrir une (certains peuvent mourir) ; si tous survivent ou au moins un revient, vous la découvrez. Ajoutez des emplacements ou construisez de l\'habitat sur une planète (+2 capacité équipage par module, 1 emplacement).',
  housing: 'Maison',
