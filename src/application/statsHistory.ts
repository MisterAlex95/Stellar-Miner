...
function loadFromStorage(): void {
  ...
  try {
    ...
    } else if (data && typeof data === 'object' && (data.recent || data.longTerm)) {
      if (Array.isArray(data.recent) && data.recent.length > 0) {
        points = data.recent.map(migratePoint);
        if (points.length > STATS_HISTORY_MAX_POINTS) points = points.slice(-STATS_HISTORY_MAX_POINTS);
        const last = points[points.length - 1];
        if (last) lastRecordAt = last.t;
      }
      if (Array.isArray(data.longTerm) && data.longTerm.length > 0) {
        longTermPoints = data.longTerm.map(migratePoint);
