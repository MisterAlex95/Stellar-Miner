...
- **presentation/** — Vue 3 root (`presentation/vue/`); mount; tabs **Mine**, **Empire**, **Research**, **Upgrades**, **Stats** (Dashboard, Research, Upgrades, Stats panels are Vue wrappers; Empire vanilla). Views (statsView, upgradeListView, planetListView, questView, comboView, progressionView, prestigeView, crewView, statisticsView, researchView, housingView). Toasts via Vue **toastStore** and **ToastContainer**. Tooltip, StarfieldCanvas, MineZoneCanvas. **presentationPortImpl** (createPresentationPort). Reusable UI in **presentation/components/**.
