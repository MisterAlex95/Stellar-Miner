...
export function createEventBadgeHtml(
  ...
  return `<span class="${cls}" data-mult="${escapeAttr(String(mult))}"><span class="event-badge__name">${escapeAttr(eventName)}</span> <span class="event-badge__mult">${escapeAttr(multStr)}</span> <span class="event-badge__time">— ${secondsLeft}s</span></span>`;
}
