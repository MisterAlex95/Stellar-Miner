...
- **data/** — JSON config: modules (catalog + planetTypes + per-module affinity), events, research, achievements, progression, balance, gameConfig, changelog
...
- **Planet affinity**: `src/application/planetAffinity.ts` — planet type (rocky/desert/ice/volcanic/gas) and affinity stats derived from planet name; data in `src/data/modules.json` (planetTypes + per-module affinity).
...
12. **Planet affinity** — Planet type (rocky/desert/ice/volcanic/gas), visuals (colors, texture, rings/belt, size) and affinity stats derived deterministically from planet name; **planetAffinity.ts** reads **planetTypes** and per-module **affinity** from **data/modules.json**.
